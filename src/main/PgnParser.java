...
public class PgnParser {
    ...
    public List<ParsedGame> parse(Reader reader, FileValidationResult fileResult) throws IOException {
        ...
        while ((line = bufferedReader.readLine()) != null) {
            ...
            if (headerMatcher.matches()) {
                ...
                if (inMoveSection && currentMoveText.length() > 0) {
                    ...
                    parsedGames.add(createParsedGame(gameIndex++, currentHeaders, currentMoveText.toString(), currentSyntaxErrors));
                ...
                currentHeaders.put(headerMatcher.group(1), headerMatcher.group(2));
                ...
                if (!inMoveSection && currentHeaders.isEmpty()) {
                    // Text before any headers - PGN standard violation
                    currentSyntaxErrors.add(new PgnErrorInfo(gameIndex, 0, trimmedLine, "Move text found before any header tags.", true));
                }
                inMoveSection = true;
        ...
        if (currentMoveText.length() > 0 || !currentHeaders.isEmpty()) {
    ...
    private ParsedGame createParsedGame(int gameIndex, Map<String, String> headers, String rawMoveText, List<PgnErrorInfo> syntaxErrors) {
        List<String> moves = extractMoveList(rawMoveText, gameIndex, syntaxErrors);
        return new ParsedGame(gameIndex, headers, moves, syntaxErrors);
    }
    ...
    public static class ParsedGame {
        ...
        public ParsedGame(int gameIndex, Map<String, String> headers, List<String> sanMoves, List<PgnErrorInfo> syntaxErrors) {
